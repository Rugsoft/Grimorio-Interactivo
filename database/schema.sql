...
--
-- La mesa de la colección del adepto (RF-05.4): TABLA NUEVA separada
-- de `favorites` a propósito — la colección y el elogio son ritos
-- distintos y cada invariante vive en su mesa. `favorites` arriba
-- queda como mesa de votos del Dominio, INTOCADA; esta tabla no
-- guarda gloria ni estado de homenaje: eso se deriva en lectura
-- (RF-04.0, estado embebido en los listados).
--
--   * `UNIQUE (user_id, spell_id)` — el sellado único (RF-01.3) como
--     invariante físico: una sola fila por hechizo y adepto en la
--     vida del tomo. Ninguna carrera de doble pestaña puede burlarlo
...
-- el adepto retira y solo la purga de cuenta arrasa. Un hechizo que
-- cae de `validated` conserva su fila con la marca solemne derivada
-- del mapa de estados (RF-03.2): la colección es memoria del adepto,
-- no espejo del catálogo (caso límite 2).
-- El guion de ascensión para bases legadas es `sql/11_grimoire_collections.sql`
