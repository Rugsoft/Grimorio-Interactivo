...
> **Ampliación ratificada (TASK-08, Tarea 1.5).** El ciclo de vida del conjuro deja de tener dos contadores. La **AUTORIDAD** es `spell_reviews.status`, que declara los cinco estados canónicos de SPEC-08 (`draft`, `experimental`, `validated`, `rejected`, `archived`). `spells.status` pasa a ser su **ESPEJO denormalizado** —se ensancha su `CHECK` a los mismos cinco estados y su comentario en `database/schema.sql` declara la relación—, y `spells.signatures_count` es espejo del contador del expediente. Ambos espejos tienen un **ÚNICO escritor**: `SpellReviewRepository`, dentro de la misma transacción que el expediente, de modo que no pueden desviarse. Las transiciones que esta especificación describe siguen siendo válidas y se delegan: `RF-05.2` (publicar) eleva el estado creando el expediente en deliberación con 0/3, y `RF-05.3` (reinicio de firmas por fraude matemático) restablece el aforo por su autoridad. Como consecuencia de admitir el quinto estado, la libreta privada del autor (`getAuthorEssays`) incorpora sus obras `rejected` —sin ellas el autor no podría leer el Dictamen de Objeción ni subsanarlas (SPEC-08 `RF-06.2`)— y deja fuera las desterradas. `spells.validation_signatures_count` NO compite con contador alguno: es el vestigio congelado del aforo génesis, jamás escrito tras las semillas, y **permanece** por formar parte del contrato JSON público de esta especificación (`validationSignaturesCount`); su retirada exigiría una revisión de contrato que aquí no se solicita. La reconciliación de una base legada vive en `sql/08_spell_status_single_source.sql`.
