...
* **RF-05.1 [Dirigido por Eventos]:**  
  ...
  * **Materialización visual ratificada (criterio con cobertura de arnés):** la cúpula luminosa es una media esfera de luz azul translúcida (`combat-dummy--dome`, animación `luminous-dome` de ~600 ms de residencia) que solo los efectos de barrera o curación alzan — el daño puro jamás — y cuya retirada corre por el bucle de escena de la vista con el reloj compartido (`domeUntil`), o al instante al restaurar. Las ataduras visuales visten la efigie por tipo de sometimiento — escarcha (`combat-dummy--bound-stun`), enredaderas rúnicas (`combat-dummy--bound-root`) y halo de lentitud orbital (`combat-dummy--bound-slow`) — sustituyéndose entre sí al cambiar el CC y disipándose con la atadura (4 s, plazo del Códice, o restauración inmediata).
...
* [ ] La cúpula luminosa se alza ante barrera o curación (jamás ante daño puro), con residencia de ~600 ms, y las ataduras visuales de CC (escarcha, enredaderas, halo de lentitud) visten y disipan con la atadura (RF-05.1, Hallazgo 13).
...
* **[RESUELTO — Hallazgo 13] Reacciones Físicas Visuales del Impacto (RF-05.1, criterio ratificado):** la cúpula luminosa y las ataduras visuales de CC dejan de ser promesa de la spec y devienen conducta exigida del maniquí. La auditoría de estabilización las encontró sin implementación ni cobertura; su materialización sigue la doctrina anti-desdoblamiento (Hallazgo 13 de SPEC-06): la retirada de la cúpula vive en el reloj compartido de la vista, jamás en un temporizador propio. El shim DOM del componente del maniquí se descubrió sin `classList` — las clases visuales se perdían en silencio en los arneses — y se hizo fiel al DOM (`add/remove/contains/toggle`). Cobertura: fase RF-05.1 de `test_combat_dummy_component.mjs` (daño no alza cúpula; barrera/curación sí, con residencia exacta de 600 ms; las tres ataduras se sustituyen entre sí; disipación a 4 s y retirada al restaurar) y verificación en vivo en la Cámara (cúpula fotografiada abrazando la efigie, atadura de hielo persistente entre páginas conforme a RF-02.3).
