...
* **RF-02.2 [Estado]:**  
  MIENTRAS un objetivo permanezca imbuido con un aura elemental, el sistema DEBERÁ mostrar un halo luminoso pulsante del color heráldico del elemento que **abraza la silueta del blanco** (criterio ratificado): una capa-luz derivada de la propia efigie —imagen clonada con desenfoque y resplandor del color vigente— en lugar de cualquier geometría circular (prohibido el anillo o disco). La duración de la ventana la declara un **contador numérico** que marca los segundos enteros restantes (5 → 0); no existe indicador rúnico circular. La capa-luz es **perenne**: en reposo pulsa en **dorado arcano** y al imbuirse se tiñe del color heráldico del elemento entrante con transición suave, regresando al dorado al expirar la ventana. **Anclaje a la efigie (criterio ratificado, Hallazgo 17):** la capa-luz DEBERÁ anclarse a la **base de la efigie** (borde inferior del contenedor de la figura del blanco) y crecer hacia arriba con una altura propia que replique la fórmula de tamaño de la efigie, de modo que silueta y efigie compartan caja vertical — jamás centrada en el anfitrión del blanco, cuyo centro vertical no coincide con el de la efigie; prohibido cualquier offset mágico dependiente del entorno.
* **RF-02.3 [Ubicuo]:**  
...
* **RF-04.2 [Dirigido por Eventos]:**  
  CUANDO se active cada reacción específica, el sistema DEBERÁ aplicar de forma simultánea al daño su respectivo **Efecto Táctico Canónico** — tanto en el backend como en la traducción al cliente (criterio ratificado, Hallazgo 15): cada efecto táctico DEBERÁ llegar al blanco del simulador con su semántica íntegra y la duración declarada por el Códice, sin que ningún eslabón del pipeline (resolutor → vista → maniquí) lo silencie ni lo deforme:
...
5. **Preferencia de Movimiento Reducido (`prefers-reduced-motion`):**  
   ...
   **Materialización ratificada (criterio con cobertura de arnés):** la corona es la clase `combo-light-crown` sobre el anfitrión del maniquí (resplandor dorado fijo de ~900 ms, declarada dentro de la media query `prefers-reduced-motion: reduce`) que la vista alza SOLO cuando la detonación queda suprimida por la preferencia — con movimiento pleno rige la deflagración bicromática del pool de partículas — y que se retira por el bucle de escena con el reloj compartido (`comboCrownUntil`), o al instante al restaurar el maniquí. El texto flotante monumental se proyecta en ambos modos.
...
* [ ] La capa-luz abraza la efigie comparta por caja vertical: anclada a su base y dimensionada con su misma fórmula de tamaño, sin desplazamiento vertical respecto a la figura (RF-02.2 ratificado, Hallazgo 17).
...
* **[RESUELTO — Hallazgo 11] Aura Contorneada, Perenne y Contador Numérico (RF-02.2, criterio ratificado):** El halo abraza la silueta del blanco: una capa-luz derivada de la propia efigie (imagen clonada con desenfoque y resplandor del color de `--aura-color`) acompaña sombrero, brazos y base del espantapájaros. La capa es **perenne**: en reposo pulsa en dorado arcano (hermana de la respiración ceremonial de la efigie) y, al impactar un conjuro elemental, se tiñe del color heráldico del elemento entrante con transición suave; al expirar la ventana regresa al dorado de reposo sin desaparecer. Toda geometría circular (anillo rúnico SVG) queda prohibida y retirada: la duración la declara exclusivamente el **contador numérico** (5 → 0), actualizado por el bucle de escena, visible solo con ventana viva. Bajo `prefers-reduced-motion` el pulso cesa pero el tinte y el contador permanecen. **[Extensión ratificada — verificación en vivo]:** El contador es indisociable de la ventana en TODA vía de extinción: la **disipación por detonación** (RF-03.4, la reacción consume el aura) DEBERÁ vaciarlo junto a la ventana, igual que la expiración natural. Sin esta exigencia, el halo descansaba en dorado con la última cifra (p. ej. «5») congelada — un oráculo mintiendo sobre una ventana muerta. Cobertura: fase 7 de `test_elemental_aura.mjs` y verificación en vivo de la ráfaga encadenada (Fuego → Agua → Viento → Fuego → Agua: Vaporización, Deflagración en Vórtice y Vaporización, con contador limpio tras cada consumo).
...
* **[RESUELTO — Hallazgo 15] Traducción Cliente de los Efectos Tácticos del Códice (RF-04.2, criterio ratificado):** Todo efecto táctico declarado en el Códice DEBERÁ traducirse íntegramente al pipeline del cliente (resolutor → vista → maniquí): un efecto presente en la matriz del backend pero ausente en la traducción de la vista se esfuma en silencio, rompiendo la paridad servidor-cliente. La auditoría de cobertura de arneses destapó dos fugas: `freezeParalysis` (Ventisca Helada, Viento+Agua) carecía de entrada en el mapa de controles de la vista, de modo que la congelación de 2 s jamás ataba al blanco; y `barrierPiercing` (Colapso Crepuscular, Luz+Oscuridad) no se traducía, de modo que el daño puro sufría la absorción prioritaria estándar — exactamente lo contrario a RF-04.2 y el Hallazgo 6 (barrera intacta, daño directo a salud). Resolución: `freezeParalysis` mapea a atadura de parálisis con la duración del Códice (2 s), y el maniquí honra la perforación (`pierceBarrier`): el daño vuela directo a la salud y el escudo sobrevive. Cobertura de regresión: la fase 9d de `test_aura_page_persistence.mjs` detona ambas reacciones en el pipeline cliente completo — congelación viva a ~1.9 s y disipada a su plazo, y perforación de un velo de 20 PV sin absorción —, cerrando la brecha de paridad que los arneses PHP no podían detectar por sí solos.
* **[RESUELTO — Hallazgo 16] Corona de Luz Estática de Combos bajo Movimiento Reducido (RNF-03, caso 5, criterio ratificado):** la auditoría de estabilización detectó que, ante `prefers-reduced-motion`, la vista suprimía la deflagración de combo y la dejaba sin sustituto visible: la reacción existía en el estado y en la bitácora, pero el blanco no mostraba rastro alguno — una ceremonia muda. Resolución: la deflagración bicromática se sustituye por la **corona de luz estática** (`combo-light-crown`, resplandor dorado fijo de ~900 ms sobre el anfitrión del maniquí, declarada dentro de la media query), alzada solo cuando la preferencia suprime el pool de partículas, retirada por el bucle de escena con el reloj compartido (`comboCrownUntil`) y cedida ante «Restaurar Maniquí»; el texto flotante monumental se proyecta en ambos modos, como exige el caso 5. De paso se corrigió un guard defectuoso que excluía la bruma de disipación bajo movimiento reducido: su deriva es sutil, no animación violenta. Cobertura de regresión: el arnés de la vista (`test_grimoire_simulator_view.mjs`) detona Vaporización Arcana real con la preferencia activa y aserta la corona alzada, la retirada a su plazo y la permanencia del anuncio solemne.
* **[RESUELTO — Hallazgo 17] Anclaje de la Capa-Luz a la Base de la Efigie (RF-02.2, criterio ratificado):** El recorrido de producción del Simulador detectó que la capa-luz del aura colgaba desplazada ~42 px por debajo de la efigie del maniquí. Causa raíz: la raíz del aura se centraba en el **anfitrión del maniquí** (top:50% + translate(-50%,-50%)) pero su altura era `auto`, y al ser todos sus hijos absolutos el bloque colapsaba a altura cero: nacía en el centro vertical del anfitrión y la silueta, descendiendo desde ahí, quedaba por debajo de la figura. Agravante: la efigie no está centrada verticalmente en su contenedor (la figura del maniquí apila barra de salud, insignias y leyenda antes de la propia efigie), de modo que el centro del anfitrión JAMÁS coincide con la vertical de la efigie. Resolución (solo hoja de estilos, sin tocar el DOM ni el componente): el anclaje correcto es la **base de la efigie** — la raíz se asienta en el borde inferior del contenedor de la figura (el mismo relleno inferior de 6 px que la figura declara) y crece hacia arriba con una altura propia que **replica la fórmula de tamaño de la efigie** (misma clamp y proporción de la imagen), de modo que silueta y efigie comparten caja vertical; horizontalmente el centro del anfitrión siempre coincide con el de la efigie (la efigie está centrada en su figura), así que el centrado puro basta y queda **prohibido cualquier offset mágico dependiente del entorno** (se descartó un offset horizontal calibrado a mano contra producción por ser artefacto del viewport). La silueta-luz se asienta sobre esa base (anclaje inferior) para abrazar sombrero, brazos y base del espantapájaros. Verificación: delta silueta vs efigie de ±1 px en reposo y tras invocación. Cobertura de regresión: fase 11 de `test_elemental_aura.mjs` y FASE 3 de `test_simulator_visual_fixes.mjs`.
