...
* **RF-03.1 [Dirigido por Eventos]:**
  ...
  *Criterios de aceptación con cobertura de arnés (ratificación de la Fase 2, Tareas 2.2–2.3):*
  * SI el bloqueo de la guardia atómica proviene de una fila histórica CUALQUIERA de esa casa para esa cuenta, ENTONCES el servicio DISCERNIRÁ en orden canónico: idempotencia primero (`APPLICATION_ALREADY_PENDING`, 409 — la petición ya remitida responde sin duplicar), clausura después (`APPLICATION_HOUSE_CLOSED`, 403 — sin consumir cupo, hallazgo 16) y cupo al final. *Evidencia: `scratch/test_clan_application_withdrawal.php` (fase 2) y `scratch/test_clan_admission_guards.php`.*
  * LA remisión declarará la motivación y el cuerpo del texto con el molde 20–500 (bordes 19/501 vedados con `INVALID_MOTIVATION`, recorte de espacios antes de medir) y la estampa de llegada solo se honrará dentro de la ventana de ±30 s — fuera de ella o ausente, manda el servidor. *Evidencia: `scratch/test_clan_application_closure.php` (fases 1 y 3).*
* **RF-03.2 [Límite]:**
...
* **RF-03.3 [Dirigido por Eventos]:**
  ...
  *Criterios de aceptación con cobertura de arnés (ratificación de la Fase 2, Tarea 2.4):*
  * CUANDO la retirada se consuma, ENTONCES la fila PERSISTIRÁ con veredicto `cancelled` y `resolved_at` fijado — liberando el cupo de pendientes y clausurando la casa vía el invariante físico del índice único, de modo que `hasSealedHouse()` pasa a `true` y la re-postulación responde `APPLICATION_HOUSE_CLOSED` (403) sin consumir cupo. *Evidencia: `scratch/test_clan_application_withdrawal.php` (fases 1–3).*
  * SI la retirada compite con el dictamen del Patriarca, ENTONCES la condición `status = 'pending'` dentro del propio UPDATE dejará un solo desenlace determinista (quien llega segundo no muta nada) y los gestos vedados (petición ajena, casa equivocada, ya resuelta) responderán sus códigos canónicos sin tocar persistencia. *Evidencia: `scratch/test_clan_application_withdrawal.php` (fases 5 y 6).*
* **RF-03.4 [Dirigido por Eventos]:**
  ...
  *Criterios de aceptación con cobertura de arnés (ratificación de la Fase 2, Tareas 2.4–2.6):*
  * CUANDO el postulante contemple un veredicto terminal propio, ENTONCES el sistema FIJARÁ `verdict_seen_at` con la estampa del contemplado y lo EX Pondrá en el contrato REST de la petición (`verdictSeenAt`, nullable, claves históricas intactas). *Evidencia: `scratch/test_clan_verdict_acknowledge.php` (fases 2 y 6).*
  * SI el contemplado se reenvía, ENTONCES el sistema RESPONDERÁ éxito SIN mutar la columna: el primer contemplado es el único instante que la historia registra (idempotencia por la triple guardia del UPDATE: terminal + propia + sin leer). *Evidencia: `scratch/test_clan_verdict_acknowledge.php` (fase 3).*
  * SI el veredicto del rechazo falta o desborda el molde de 20–500 caracteres, ENTONCES el sistema RESPONDERÁ 400 `INVALID_VERDICT_MOTIVE` con su `recoveryAction` propio — código canónico separado de `INVALID_MOTIVATION`, pues el dictamen es acto del lado deliberante y no una petición del postulante — sin consumir el dictamen (la petición sigue `pending`). *Evidencia: `scratch/test_clan_verdict_motive.php` (fase 1).*
  * CUANDO el rechazo porte su motivo canónico, ENTONCES el sistema INSCRIBIRÁ el asiento `CLAN_APPLICATION_VERDICT` con identidad del Patriarca, estampa temporal y motivo íntegro (Artículo III.3, hallazgo 23), y la aprobación — que no exige motivo, pues el ingreso ES su motivo — inscribirá el suyo y un asiento `CLAN_APPLICATION_RESIDUALS_ANNULLED` por cada petición huérfana (RF-03.7). *Evidencia: `scratch/test_clan_verdict_motive.php` (fases 2 y 3).*
...
* **RF-04.4 [Dirigido por Eventos — Reparto por Actor]:**
  ...
  *Criterios de aceptación con cobertura de arnés (ratificación de la Fase 2, Tareas 2.4–2.6):*
  * La retirada del postulante INSCRIBIRÁ su asiento `CLAN_APPLICATION_WITHDRAWN` con identidad, alias y rol del POSTULANTE, objetivo `clan` y la casa nombrada en la justificación; el acto vivirá en el catálogo cerrado de `AuditEntry` con su rótulo castellano «Retirada de una Petición de Ingreso». *Evidencia: `scratch/test_clan_application_withdrawal.php` (fase 4).*
  * El dictamen INSCRIBIRÁ su asiento `CLAN_APPLICATION_VERDICT` («Dictamen sobre una Petición de Ingreso») con identidad del LADO DELIBERANTE — nunca del postulante — tanto en el rechazo con motivo como en la aprobación; y cada residual anulada dejará su asiento `CLAN_APPLICATION_RESIDUALS_ANNULLED` («Anulación de Peticiones Huérfanas») desde la casa aprobadora. *Evidencia: `scratch/test_clan_verdict_motive.php` (fases 2 y 3) y `scratch/test_moderation_audit_integration.php` (aserto de catálogo cerrado con rótulos de SPEC-03).*
* **RF-04.5 [Ubicuo]:**
